...
import ctypes
ctypes.CDLL('/usr/lib/x86_64-linux-gnu/libnvvm.so', mode=ctypes.RTLD_GLOBAL)
...
def train_scanpath_video_predictor(train_data, val_data, model, device, criterion, lr = 0.001, EPOCHS=10, model_name='Model'):
    ...
    for epoch in range(EPOCHS): # Training loop
        ...
        for x, y in train_data:
            model.zero_grad()
            outputs = []
            gaussian_map = y[:, 0, :, :, :].numpy()
            point = sg.generate_image_probabilistic_saliency_scanpath(gaussian_map, 4)
            gaussian_map= gmg.gaussian_map(gaussian_map.shape[2], gaussian_map.shape[3], (point[1],point[0]))
            ...
            total_loss_DTW = 0
            start_idx = max(0, pred.shape[1] - config.n_last_sequences)
            for i in range(start_idx, pred.shape[1]-1): # [b_size(1), len, H, W]
                loss = criterion(pred[:, i, :, :, :], y[:, i+1, :, :, :].to(device))
                total_loss_DTW = total_loss_DTW + loss
            total_loss_DTW = total_loss_DTW / (pred.shape[1]-start_idx)
        ...
        if epoch % config.ckp_nepochs == 0: # Save checkpoint and model every config.ckp_nepochs epochs
...
# PS: Probabilistic Scanpath
def train_scanpath_video_predictor_with_PS(train_data, val_data, model, device, criterion, lr = 0.001, EPOCHS=10, model_name='Model'):
    writer = SummaryWriter(os.path.join(config.runs_data_dir, model_name +'_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + '/'))
    path = os.path.join(config.models_dir, model_name + '_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    os.makedirs(path, exist_ok=True)
    ...
    for epoch in range(EPOCHS): # Training loop
        ...
        for x, y in train_data:
            ...
            gaussian_map = y[:, 0, random_idx, :, :, :].numpy()
            frame_del_video = x[:, 0, :, :, :]
            frame_with_coords = torch.cat((frame_del_video.to(device), coordconv_matrix.to(device), gaussian_map.to(device)), dim=1)
            state_e, state_d = model.init(frame_with_coords)
            ...
            total_loss_DTW = 0
            start_idx = max(0, pred.shape[1] - config.n_last_sequences)
            for user_idx in range(y.shape[2]):  # Iterate over each user/scanpath
                for i in range(start_idx, pred.shape[1]-1): # [b_size(1), len, H, W]
                    loss = criterion(pred[:, i, :, :, :], y[:, i+1, user_idx, :, :, :].to(device))
                    total_loss_DTW = total_loss_DTW + loss
            total_loss_DTW = total_loss_DTW / (pred.shape[1] * y.shape[2])
