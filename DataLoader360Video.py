...
class RGB_with_PS(Dataset): # PS: Probabilistic Scanpath
    def __init__(self, path_to_frames,path_to_gaze_data, video_names, frames_per_data=20, split_percentage=0.2, split='train', resolution = [240, 320], skip=20, load_names=False, transform=False, inference=False):
        self.sequences = []
        self.correspondent_sal_maps = []
        self.frames_per_data = frames_per_data
        self.path_frames = path_to_frames
        self.path_to_gaze_data = path_to_gaze_data
        self.resolution = resolution
        self.load_names = load_names
        self.transform = transform
        ...
        for name in video_names:
            video_frames_names = os.listdir(os.path.join(self.path_frames, name))
            video_frames_names = sorted(video_frames_names, key=lambda x: int((x.split(".")[0]).split("_")[2]))

            # Frame number and saliency name must be the same (ex. frame name: 0001_00_0023.png, saliency map: 0023.png)
            ...
            # Group frames by scanpath
            scanpath_groups = {}
            for frame in video_frames_names:
                scanpath_id = frame.split("_")[1]
                if scanpath_id not in scanpath_groups:
                    scanpath_groups[scanpath_id] = []
                scanpath_groups[scanpath_id].append(frame)

                # Split the videos in sequences of equal length for each scanpath
                for scanpath_id, frames in scanpath_groups.items():
                    for end in range(initial_frame, len(frames), frames_per_data):

                        # Check if exist the ground truth saliency map for all the frames in the sequence
                        valid_sequence = True

                        if not self.path_to_gaze_data is None:

                            for frame in frames[sts:end]:
                                if not os.path.exists(os.path.join(self.path_to_gaze_data, frame.split("_")[0], frame.split("_")[2])):
                                    valid_sequence = False
                                break

                        if valid_sequence: self.sequences.append(frames[sts:end])
                        sts = end
                        if inference: sts = sts - 4 # To overlap sequences while inference for smooth predictions (4 frames) 
    ...
    def __getitem__(self, idx):
        ...
        for frame_name in self.sequences[idx]:
            ...
            if not self.path_to_gaze_data == None:
                scanpath_id = frame_name.split("_")[1]
                sal_map_paths = [os.path.join(self.path_to_gaze_data, frame_name.split("_")[0], scanpath_id, frame_name.split("_")[2]) for scanpath_id in os.listdir(os.path.join(self.path_to_gaze_data, frame_name.split("_")[0]))]
                saliency_imgs = []
                for sal_map_path in sal_map_paths:
                    assert os.path.exists(sal_map_path), 'Saliency map has not been found in path: ' + sal_map_path
                    ...
                    saliency_img = (saliency_img - np.min(saliency_img)) / (np.max(saliency_img) - np.min(saliency_img))
                    saliency_img = torch.FloatTensor(saliency_img).unsqueeze(0)
                    saliency_imgs.append(saliency_img.unsqueeze(0))
                labels.append(torch.cat(saliency_imgs, 0))

        if self.load_names:
            if self.path_to_gaze_data is None: sample = [torch.cat(frame_img, 0), frame_names]
            else: sample = [torch.cat(frame_img, 0), torch.cat(labels, 0), frame_names]
        else:
            if self.path_to_gaze_data is None: sample = [torch.cat(frame_img, 0)]
            else: sample = [torch.cat(frame_img, 0), torch.cat(labels, 0)]
