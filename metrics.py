"""
  code from:
    https://github.com/DaniMS-ZGZ/saliency/blob/master/metrics/metrics.py
"""
...
def NSS(saliency_map, fixation_map):
    ...
    if saliency_map.size != fixation_map.size:
      saliency_map = np.array(Image.fromarray(saliency_map).resize(fixation_map.shape[0],fixation_map.shape[1]))
...
def CC(saliency_map, saliency_map_gt):
    ...
    if saliency_map.size != saliency_map_gt.size:
      saliency_map = np.array(Image.fromarray(saliency_map).resize(saliency_map_gt.shape[0],saliency_map_gt.shape[1]))
